...
const App = () => {
  ...
  const totalDroid =
    (data?.results && data?.results.length) - (totalHuman && totalHuman.length);
        ...
        if (row?.species && row?.species.length > 0) {
          return (
            <div>
              <FontAwesomeIcon
                icon={faEye}
                onClick={() => openModal(row?.species[0])}
              />
            </div>
          );
        } else {
