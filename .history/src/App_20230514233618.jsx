...
const App = () => {
 ...
 const totalDroid =()=> {
  const total = data?.results&& data?.results.reduce((acc, sum)=> {
    let count;
    if(acc?.name==="male"||acc?.name==="female"){
      count+=1
    }
    console.log(count)
  })
...
<Card cardName="Total Results" count={data?.result&&data?.result.length}/>
<Card cardName="Total Droid" count={4}/>
